import { 
  Point, distanceBetween, makeSvgContainer, makeCircle, makeLine, makeText 
} from './svg-utils';
...
const defaultOpts: Omit<Opts, 'id'> = {
  ...
  label: '',
  onClick: (_coord: FretCoord, _diagram: FretboardDiagram) => null,
}
...
export class FretboardDiagram {
  ...
  // the (x, y) location of each FretCoord
  points: Point[];
  ...
  drawDot(dot: Dot) {
    ...
    const dotCircle = makeCircle(this.svgElem, x, y, r, color);
    return dotCircle;
  }
